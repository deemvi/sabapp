<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Registrasi User</title>
    <link rel="stylesheet" href="../assets/css/log-style.css">
</head>
<body>
    <div class="container-regis">
        <h2>Daftar Untuk Login</h2>
        <form action="<?= site_url('auth/registrasi_user') ?>" method="post">
            <label for="username">Username :</label><br>
            <input type="text" name="username" placeholder="Isi Username">
            <br>
            <label for="password">Password :</label><br>
            <input type="password" name="password" placeholder="Isi Password">
            <br>
            <label for="email">Email Anda :</label><br>
            <input type="email" name="email" placeholder="Isi Email Anda">
            <br>
            <label for="nama_user">Nama Anda :</label><br>
            <input type="text" name="nama_user" placeholder="Isi Nama Anda">
            <br>
            <input type="submit" value="Tambah" name="btn_regis" class="btn">
        </form>
    </div>
</body>
</html>